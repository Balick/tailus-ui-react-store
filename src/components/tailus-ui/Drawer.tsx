...
import { title, text, type TitleSizeProp, type TextProps, type TextSizeProp, type TextAlignProp, type TextWeightProp } from '@tailus/themer';
...
>(({ className, fancy, mixed, ...props }, forwardedRef) => {
  ...
  return <Primitive.Content {...props} ref={forwardedRef} className={content({ fancy, mixed, direction, withControler, className })} />;
});

const Overlay = React.forwardRef<React.ElementRef<typeof Primitive.Overlay>, React.ComponentProps<typeof Primitive.Overlay>>(
  ({ className, ...props }, forwardedRef) => {
    const { overlay } = drawer();

    return <Primitive.Overlay {...props} ref={forwardedRef} className={overlay({ className })} />;
  }
);
...
>(({ className, size = 'base', align, weight = 'medium', ...props }, forwardedRef) => (
  <Primitive.Title {...props} ref={forwardedRef} className={title({ size, align, weight, className })} />
));
...
>(({ className, size, weight, align, neutral, ...props }, forwardedRef) => (
  <Primitive.Description {...props} ref={forwardedRef} className={text({ size, weight, align, neutral, className })} />
));
...
export { Root, Trigger, NestedRoot, Portal, Close, Content, Overlay, Title, Description };
